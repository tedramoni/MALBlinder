...
function validateUser(user) {
    ...
    $.ajax({
        type: 'GET',
        url: "https://cors.now.sh/https://myanimelist.net/malappinfo.php",
        data: {
            u: user
        },
        dataType: "xml",
        ...
        success: function (xml) {
            test = $(xml).find('myinfo').first().find('user_name').first().text() == user ? true : false;
...
$(document).ready(function () {
    ...
    function random_generate() {
       ...
       */
        ...
        $('.titleQuote').text(anime_quote[random_number].quoteanime);
    }
